...
statement: designator (ASSIGN expr | LPAREN (actPars)? RPAREN | INCREMENT | DECREMENT) SEMICOLON
         ...
         | block
         | SEMICOLON
         | LIST LESS_THAN IDENTIFIER GREATER_THAN IDENTIFIER (ASSIGN listFactor)?; // SE AGREGO PARA DECLARAR LISTAS DENTRO DEL PARSER, SE DECLARARON LOS TOKENS EN EL SCANNER.
...
listFactor: NEW LIST LESS_THAN IDENTIFIER GREATER_THAN LPAREN RPAREN; // SE AGREGO PARA DECLARAR LISTAS DENTRO DEL PARSER, SE DECLARARON LOS TOKENS EN EL SCANNER.
...
factor: designator (LPAREN (actPars)? RPAREN)?
      ...
      | CHAR_CONSTANT
      | STRING_CONSTANT
      | DOUBLE_CONST //SE AGREGO PARA MANEJAR TIPOS DOUBLE.
      | (TRUE | FALSE)
      | NEW IDENTIFIER (LBRACK RBRACK| LBRACK expr RBRACK)? //SE AGREGO UNA FUNCIONALIDAD MAS PARA CUANDO SE HACE UN NEW DE UN IDENTIFIER SE PUEDA O NO ESTABLECER UNA EXPRESION DENTRO DE BRACKS.
      | LPAREN expr RPAREN;
